...
If neigther "-l" nor "-c" option is specified, plot_energyplus tries to open the latest directory under "${ENERGYPLUS_LOG}" directory.
...
5. Electric demand power (whole building, facitliy, HVAC)
6. Reward
...
necessarify same as the reward value computed by our reward function.

You can pan or zoom each graph by entering pan/zoom mode by clikcing cross-arrows on the bottom left of the window.
